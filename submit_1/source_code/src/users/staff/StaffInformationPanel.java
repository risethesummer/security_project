...
public class StaffInformationPanel extends JPanel {
    ...
    public Staff getUpdate()
    {
        ...
        java.util.Date dob = !newDob.getDateModel().getValue().before(Date.from(Instant.now())) ? Date.valueOf(oldDob.getText()) : newDob.getDateModel().getValue();
        String idCard = newIdCard.getText().isBlank() ? null : newIdCard.getText();
